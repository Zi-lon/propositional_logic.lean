...
lemma raa_provable {Γ : Set (Fml α)} {ϕ : Fml α} (h :(Γ ∪ {¬ϕ}) ⊢  ⊥ ) :
    (Γ ⊢ ϕ) := by
      ...
      · have neg_phi_in_delta : (¬ϕ) ∈ Δ := by
          simp at h_sub
          simp_all only [Set.union_singleton]
          by_contra ϕ_in_Δ
          apply g
          intro ϕ hϕ
          have h_in_Γ := h_sub hϕ
          simp at h_in_Γ
          rcases h_in_Γ with rfl | h_in_Γ
          trivial
          trivial
        let Δ' := Δ\{¬ϕ}
        have f : Δ' ⊆ Γ := by
          ...
          simp_all only [Set.union_singleton]
          intro ψ hψ
          rcases hψ with ⟨h_in_Δ, h_neq⟩
          have h_in_Γ := h_sub h_in_Δ
          rcases h_in_Γ with rfl | h_in_Γ
          trivial
          assumption
        have aux: (Δ' ∪ {¬ϕ}) = Δ := by
    ...
    Γ ⊢ (ϕ → ψ) := by
      ...
      · have h_eq : Δ = Δ' := by
        ...
        have der_final : Der Δ (ϕ → ψ) := by
         apply Der.imp_intro der_phi
        rw [←h_eq] at g
        exact ⟨Δ, g, Nonempty.intro der_final⟩
